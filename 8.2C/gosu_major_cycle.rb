...
class GameWindow < Gosu::Window
    ...
    def initialize
        ...
        earth_file = File.join(media_dir, "earth.png")
        bug_file = File.join(media_dir, "bug.png")

        # Create and load images to display
        @background_image = Gosu::Image.new(earth_file)
        @img = Gosu::Image.new(bug_file)

        # Create and load a font for drawing text on the screen
        @font = Gosu::Font.new(20)
        @cycle = 0
        @shape_x = 0 # initialize shape_x at 0
    ...
    def draw
        ...
        @font.draw_text("Cycle count: #{@cycle}", 10, 10, z = ZOrder::TOP, 1.0, 1.0, Gosu::Color::WHITE)
        @img.draw(@shape_x, 30, ZOrder::TOP)
        puts("2. In draw\n")
    end
end
