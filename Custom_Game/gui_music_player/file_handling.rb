...
def read_albums(dir, album_dirs)
    albums = Array.new()
    count = album_dirs.length
    index = 0
    while index < count
        sub_dir = "#{dir}/album#{index}/"
        album = read_album(sub_dir, index)
        albums << album
        index += 1
    end
    return albums
...
def load_albums()
    dir = 'music'
    album_dirs = Dir.children(dir)
    albums = read_albums(dir, album_dirs)
    return albums
end
